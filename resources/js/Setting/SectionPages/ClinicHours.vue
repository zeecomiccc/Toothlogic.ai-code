<template>
  <form @submit="formSubmit" class="clinic-hour">
    <div class="col-md-12 d-flex justify-content-between">
      <CardTitle :title="$t('setting_sidebar.lbl_clinic')" icon="fa-solid fa-clock"></CardTitle>
    </div>
    <div class="form-group col-12" v-if="service_providers.options.length > 1">
      <label class="form-label">{{ $t('setting_clinic_hours.lbl_name') }}<span class="text-danger">*</span></label>
      <Multiselect id="service_provider_id" v-model="service_provider_id" v-bind="singleSelectOption" :options="service_providers.options" @select="serviceProviderSelect" class="form-group"></Multiselect>
      <span class="text-danger">{{ errors.service_provider_id }}</span>
    </div>
          ...
          <div class="col-md-12 row row-cols-3">
            ...
            <div class="form-group">
              <div class="d-flex gap-1">
                ...
                <label class="form-label" :for="`${index}-dayoff`">{{ $t('setting_clinic_hours.lbl_add_day_off') }}</label>
              </div>
            </div>
          </div>
          <template v-if="!day.is_holiday">
            <div v-for="(input, index) in day.breaks" :key="index" class="form-group">
              <h6>{{ $t('setting_clinic_hours.lbl_break') }}</h6>
              <div class="col-md-12 row row-cols-3">
            ...
            <div>
              <a @click="addInputField(day)" class="clickable-text">
                <h6><i class="fa fa-plus-circle" aria-hidden="true"></i> {{ $t('setting_clinic_hours.lbl_add_break') }}</h6>
              </a>
            </div>
          </template>
        </div>
...
import { LISTING_URL, STORE_URL } from '@/vue/constants/clinic_hours'
...
const { value: service_provider_id } = useField('service_provider_id')
...
const serviceProviderSelect = (e) => {
  const serviceProviderId = service_provider_id.value

  getRequest({ url: LISTING_URL, id: { service_provider_id: serviceProviderId } }).then((res) => {
    if (res.status) {
      if (res.data != '') {
...
const formSubmit = handleSubmit((values) => {
  ...
  values.service_provider_id = service_provider_id.value
