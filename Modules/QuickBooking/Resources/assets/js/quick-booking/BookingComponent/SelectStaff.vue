<template>
    <div class="card-list-data">      
      <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3" v-if="!IS_LOADER">
          <template v-for="(item, index) in doctorList" :key="`items-${index}`">
              <div class="col">
                <div class="iq-widget">
                    <input type="radio" :id="item.first_name + item.doctor_id" v-model="doctor_id" :value="item.doctor_id" name="radio" class="btn-check" @change="onChange"/>
                    <label :for="item.first_name + item.doctor_id" class="d-block w-100">
      ...
      <div class="h-100 w-75 d-flex align-items-center justify-content-center mx-auto" v-if="doctorList.length == 0">
    ...
    <div class="card-footer">
      ...
      <button type="button" v-if="doctorList.length > 0 && wizardNext" class="btn btn-primary iq-text-uppercase" :disabled="doctor_id !== null ? false : true" @click="nextTabChange(wizardNext)">Next</button>
    </div>
</template>
<script setup>
...
// Select Options List Request
 import { DOCTOR_LIST } from '@/vue/constants/quick_booking'
...
const getdoctors = () => {
  ...
  const data = {
    clinic_id: store.booking.clinic_id,
    service_id: store.booking.services[0].service_id,
  }
  
  if(data.clinic_id !== null || data.service_id !== null || props.user_id !==null) {
    listingRequest({ url: DOCTOR_LIST, data: { user_id: props.user_id, service_id:data.service_id, clinic_id:data.clinic_id}}).then((res) => {
      IS_LOADER.value = false
      doctorList.value = res.data
    })
  }
}
watch(() => store.booking.clinic_id,() => {
  getdoctors()
}, {deep: true})

watch(() => doctor_id.value, (value) => {
  store.updateBookingEmployeeValues(value)
  store.updateBookingValues({key: 'doctor_id',value: value})
})
