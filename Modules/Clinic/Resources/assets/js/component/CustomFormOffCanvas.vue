<template>
  <form @submit="formSubmit" class="bussiness-hour">
    <div class="offcanvas offcanvas-end offcanvas-w-40" tabindex="-1" id="customform-offcanvas" aria-labelledby="form-offcanvasLabel">
        ...
        <div v-for="(element, index) in field_data" :key="element.id" class="form-field">
          ...
          <div v-else-if="element.type === 'checkbox'" :ref="'field_' + element.id">
            ...
            <div v-for="(option, index) in element.option" :key="index" class="form-check">
              <input type="checkbox" :id="'checkbox_' + element.id + '' + index" :value="option" :checked="Array.isArray(element.value) && element.value.includes(option)" @change="handleCheckboxChange($event, element, option)" class="form-check-input" />
              <label :for="'checkbox' + element.id + '_' + index" class="form-check-label">
                {{ option }}
              </label>
            </div>
          </div>
          ...
          <div v-else-if="element.type === 'radio'" :ref="'field_' + element.id">
            ...
            <div v-for="(option, optIndex) in element.option" :key="optIndex" class="form-check">
              <input type="radio" :id="'radio_' + element.id + '_' + option" :value="option" v-model="element.value" class="form-check-input" />
    ...
    listingRequest({ url: GET_CUSTOM_FORM, data: { form_id: formId.value, appointent_id: appointmentId.value, type: appointmentType.value } })
      .then((res) => {
        if (res.data) {
          ...
          field_data.value.forEach((element) => {
            if (element.type === 'radio' && !element.value) {
              element.value = element.option[0] // Default to the first option
            }
          })
        }
      })
...
const handleCheckboxChange = (event, element, option) => {
  ...
  if (event.target.checked) {
    ...
    element.value = element.value.filter((item) => item !== option)
  }
}
// message
...
const reset_datatable_close_offcanvas = (res) => {
  if (res.status) {
    window.successSnackbar(res.message)
    bootstrap.Offcanvas.getInstance('#customform-offcanvas').hide()
  } else {
    window.errorSnackbar(res.message)
...
const handleRadioChange = (element, option) => {
  ...
  () => {
    isFormValid.value = field_data.value.every((field) => {
      if (field.type === 'radio' && !field.value) {
        field.value = field.option[0] // Set the first option as default
      }
      if ((field.validation_status === 1 || field.validation_status === true) && field.type !== 'hr_tag' && field.type !== 'heading') {
