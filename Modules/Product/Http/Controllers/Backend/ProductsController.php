...
{
    // use Authorizable;
    protected string $exportClass = '\App\Exports\ProductExport';
    ...
    public function index(Request $request)
    {
        ...
        $module_action = 'List';
        $columns = CustomFieldGroup::columnJsonValues(new Product());

        $customefield = CustomField::exportCustomFields(new Product());
        $export_import = true;
        $export_columns = [
            [
                'value' => 'name',
        ...
        return view('product::backend.products.index_datatable', compact('module_action', 'filter', 'export_import', 'export_columns', 'export_url', 'brands', 'categories', 'customefield', 'columns'));
    }
            ...
            if ($product->has_variation) {
                ...
                if ($stock_qty > 0) {
                    $data[] = [
                        ...
                        'text' => $product->name,
                        'extra_data' => json_encode(['variation_id' => $first_variation->id, 'product_id' => $product->id, 'qty' => $stock_qty, 'price' => $first_variation->price, 'variation_name' => null]),
                    ];
                }
            }
        }
    ...
    public function index_data(Request $request, Datatables $datatable)
    {
        ...
        return $datatable->eloquent($query)
                        ...
                        ->rawColumns(['action', 'status', 'image','name', 'check', 'categories', 'is_featured'])
                        ->make(true);
                         // Custom Fields For export
                        $customFieldColumns = CustomField::customFieldData($datatable, Product::CUSTOM_FIELD_MODEL, null);
    ...
    public function store(ProductRequest $request)
    {
        ...
        if ($request->file_url) {
            storeMediaFile($product, $request->file('file_url'));
        }
    ...
    public function update(ProductRequest $request, $id)
    {
        ...
        if ($request->file_url == null) {
            $product->clearMediaCollection('file_url');
        }
    ...
    public function bulk_action(Request $request)
    {
        ...
        switch ($actionType) {
            ...
            case 'change-status':
                $products = Product::whereIn('id', $ids)->update(['status' => $request->is_featured]);
