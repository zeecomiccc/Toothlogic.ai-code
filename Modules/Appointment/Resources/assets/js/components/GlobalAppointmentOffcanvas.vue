<template>
  <form @submit="formSubmit">
    <div class="offcanvas offcanvas-end offcanvas-w-40" tabindex="-1" id="appointment-form-offcanvas"
          ...
          <div class="col-12">
            ...
            <div class="row" v-if="selectedPatient">
              <div class="form-group">
                ...
                <div v-if="showAdditionalPatient" class="mt-2">
                  ...
                  <!-- <div v-if="otherpatientlist.list.length > 0" class="d-flex flex-wrap gap-2">
                    <div v-for="patient in otherpatientlist.list" :key="patient.id" class="book-for-appointments" :class="{ 'bg-primary text-white border-primary active': selectedPatientId === patient.id }" @click="selectPatient(patient.id)">
                        <img :src="patient.profile_image" class="img-fluid rounded-circle avatar-35 object-fit-cover" />
                        <h6 class="appointments-title">{{ patient.first_name }}</h6>
                    </div>
                  </div> -->

                  <div v-if="otherpatientlist.options.list && otherpatientlist.options.list.length"
                    ...
                    <div v-for="patient in otherpatientlist.options.list" :key="patient.id"
                      class="book-for-appointments" :class="{
                        'bg-primary border-primary active': selectedPatientId === patient.id
              ...
              <div v-if="!isDoctor" class="col-md-6 form-group">
                <label class="form-label">{{ $t('clinic.lbl_select_doctor') }} <span
                    class="text-danger">*</span></label>
              ...
              <div class="col-md-12 form-group appointment_date">
                ...
                <flat-pickr :placeholder="$t('clinic.lbl_appointment_date')" id="appointment_date" class="form-control"
                  v-model="appointment_date" :config="config"
              ...
              <div class="col-md-12 form-group">
                <div class="d-flex justify-content-between align-items-center">
                  <label class="form-label" for="availble_slot">{{ $t('clinic.lbl_availble_slots') }} <span
                      class="text-danger">*</span></label>
                </div>
  ...
  <div class="modal fade" id="exampleModal1" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="exampleModalLabel">{{ $t('clinic.add_other_patient') }}</h1>
        ...
        <div class="modal-body">
          <form @submit="submitForm">
            <div class="row">
              ...
              <div class="col-md-8">
                <div class="mb-3">
                  <label for="firstName" class="form-label">{{ $t('clinic.lbl_first_name') }}</label>
                ...
                <div class="mb-3">
                  <label for="lastName" class="form-label">{{ $t('clinic.lbl_last_name') }}</label>
                ...
                <div class="mb-3">
                  <label for="dob" class="form-label">{{ $t('clinic.date_of_birth') }}</label>
                  <flat-pickr v-model="dob" :config="dobConfig" class="form-control" id="dob"
                    :placeholder="$t('clinic.date_of_birth')"> </flat-pickr>
                  <span id="dobError" class="text-danger"></span>
                </div>

                <div class="mb-3">
                  <label for="phoneNumber" class="form-label">{{ $t('clinic.lbl_contact_number') }}</label>
                  <div class="input-group">
                    <vue-tel-input class="w-100" @input="handleInput" v-bind="{ mode: 'international', maxLen: 15 }"
                      autocomplete="new-password" :inputOptions="{
                        placeholder: $t('employee.lbl_phone_number_placeholder')
                      }"></vue-tel-input>
...
import { EDIT_URL, STORE_URL, UPDATE_URL, PATIENT_OTHER_STORE_URL, CLINIC_LIST, DOCTOR_LIST, SERVICE_LIST, OTHERPATIENT_LIST, PATIENT_LIST, SERVICE_PRICE, TAX_DATA, GET_AVALABLE_SLOT, SAVE_PAYMENT } from '../constant/clinic-appointment'
...
import 'vue3-tel-input/dist/vue3-tel-input.css' // Add this line
// props
const props = defineProps({
  ...
  'booking-type': { type: String, default: '' }
})
...
const getotherpatient = (value) => {
  useSelect({ url: OTHERPATIENT_LIST, data: value }, { value: 'user_id', label: 'first_name' }).then((data) => {
...
const getServiceList = (value) => {
  ...
  useSelect({ url: SERVICE_LIST, data: { doctor_id: value, clinic_id: clinic_id.value } }, { value: 'id', label: 'name' }).then((data) => {
...
useOnOffcanvasHide('appointment-form-offcanvas', () => {
  IS_SUBMITED.value = false
  setFormData(defaultData())
})
useOnOffcanvasShow('appointment-form-offcanvas', () => {
  IS_SUBMITED.value = false
})
...
const submitForm = () => {
  ...
  const isValid = firstName.value && lastName.value && dob.value && phoneNumber.value && gender.value && relation.value
  if (!isValid) return
